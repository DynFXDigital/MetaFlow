...
- **Classifier (CLS)**: Determines `live-ref` vs `materialized` per artifact type and injection config.
...
- **Settings Injector (SI)**: Writes/removes VS Code settings for live-referenced artifact paths.
...
- Materializer consumes the `OverlayResult`: writes materialized files with provenance headers, updates managed state, injects settings for live-referenced files.
...
| DES-0105 | Classifier | Classifies artifacts as `live-ref` or `materialized` per directory prefix and injection overrides | REQ-0109, REQ-0110, REQ-0111, REQ-0112 | `src/engine/classifier.ts` |
...
| DES-0204 | Settings Injector | Computes VS Code settings entries for live-ref directories and hook paths; supports clean removal | REQ-0109, REQ-0110 | `src/engine/settingsInjector.ts` |
...
| DES-0305 | Files TreeView | Groups effective files by classification (Live-Referenced / Materialized); shows source layer | REQ-0403 | `src/views/filesTreeView.ts` |
