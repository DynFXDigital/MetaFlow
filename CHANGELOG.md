...
- `metaflow codex-support-boundaries --runtime-evidence-review-queue` now emits focused review-only runtime evidence triage documents for terminal and CI review, including waived, expired, and stale adapter evidence queues.
- Codex waived, expired, and stale-adapter runtime evidence review queues now include advisory review items for the matching concepts without adding release gate blockers.
...
- `MetaFlow: Open Codex Runtime Evidence Review Queue` opens focused runtime evidence triage documents from the VS Code command palette for all runtime-only concepts, release-ready blockers, individual gate queues, waived evidence, expired evidence, and stale adapter evidence queues.
...
- Codex Windows platform runtime support now distinguishes platform intent from native sandbox selection, administrator setup, enterprise requirements, WSL2 execution, session read grants, and sandbox enforcement proof.
- Codex Linux platform runtime support now distinguishes platform intent from `bubblewrap`, user namespace, AppArmor, writable root, package-manager, WSL2 distribution, repository-location, and sandbox enforcement proof.
- Codex macOS platform runtime support now distinguishes platform intent from Seatbelt sandbox enforcement, Codex app installation, workspace opening, macOS Privacy & Security permissions, local environment actions, managed preferences, and sandbox proof.
- Codex local environment runtime support now distinguishes worktree setup-script and project-action intent from Codex app settings, app-local environment state, integrated-terminal execution, dependency installation, and runtime proof.
- Codex non-interactive runtime support now distinguishes scripted `codex exec` intent from live credentials, sandbox and approval posture, JSON or schema output, session resume state, repository trust checks, and runtime proof.
- Codex SDK runtime support now distinguishes SDK integration intent from package installation, language-runtime provisioning, app-server startup, SDK client initialization, credential selection, thread lifecycle, sandbox presets, embedding application deployment, traces, and runtime proof.
- Codex app-server runtime support now distinguishes app-server integration intent from process startup, transport selection, WebSocket listener authentication, JSON-RPC client initialization, thread and turn lifecycle, event-stream handling, schema generation, overload handling, and runtime proof.
- Codex IDE extension runtime support now distinguishes IDE-extension intent from extension installation, editor focus, open-file and selected-text context, Command Palette actions, file tagging, model selection, WSL settings, cloud preview, continue-local workflows, and runtime proof.
