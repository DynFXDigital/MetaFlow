# Claude Code metadata references

Last reviewed: 2026-05-22

## Anthropic Claude Code docs (authoritative)

- Memory system (CLAUDE.md, Auto Memory, Rules): https://code.claude.com/docs/en/memory.md
- Sub-agents: https://code.claude.com/docs/en/sub-agents.md
- Skills: https://code.claude.com/docs/en/skills.md
- Hooks guide: https://code.claude.com/docs/en/hooks-guide.md
- Hooks reference: https://code.claude.com/docs/en/hooks.md
- Settings and configuration: https://code.claude.com/docs/en/settings.md
- MCP integration: https://code.claude.com/docs/en/mcp.md
- Documentation map: https://code.claude.com/docs/en/claude_code_docs_map.md
- Documentation index: https://code.claude.com/docs/llms.txt

## Repo-local research and design inputs

- `capabilities/metadata-authoring/github-copilot-metadata-authoring/README.md`
- `capabilities/metadata-authoring/codex-metadata-authoring/README.md`# Claude Code metadata references

Last reviewed: 2026-03-28

## Anthropic Claude Code docs (authoritative)

- Memory system (CLAUDE.md, Auto Memory, Rules): https://code.claude.com/docs/en/memory.md
- Sub-agents: https://code.claude.com/docs/en/sub-agents.md
- Skills: https://code.claude.com/docs/en/skills.md
- Hooks guide: https://code.claude.com/docs/en/hooks-guide.md
- Hooks reference: https://code.claude.com/docs/en/hooks.md
- Settings and configuration: https://code.claude.com/docs/en/settings.md
- MCP integration: https://code.claude.com/docs/en/mcp.md
- Documentation map: https://code.claude.com/docs/en/claude_code_docs_map.md

## Repo-local research and design inputs

- `capabilities/metadata-authoring/github-copilot-metadata-authoring/README.md`
- `capabilities/metadata-authoring/codex-metadata-authoring/README.md`
