...
- Keep always-on agent instruction surfaces minimal: put invocation triggers, scope, precedence, and must-follow rules in the file, then delegate procedures and edge cases to skills or support docs.
...
- Do not duplicate long workflows in `AGENTS.md` when a stable nearby skill or support doc can carry that detail.
...
- When documenting interactive workflows, do not stop at “use askQuestions.” State the trigger for asking, the smallest-useful batching rule, the preference for recommended defaults, and the one-question chat fallback when the tool is unavailable.
