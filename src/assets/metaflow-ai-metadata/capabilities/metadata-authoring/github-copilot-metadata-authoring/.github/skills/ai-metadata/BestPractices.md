...
- Keep GitHub-facing prompts portable across supported IDE surfaces; do not assume a prompt will run on the GitHub web chat surface.
...
- Use `target` only when the file intentionally differs between VS Code and GitHub Copilot execution surfaces.
...
- Keep the skill `name` aligned with the parent directory name and avoid manual namespace prefixes; plugin packaging adds prefixes automatically where needed.
...
- Prefer repository hook files under `.github/hooks/*.json` as the shared portability baseline.
- Use inline repository settings hooks or plugin-bundled hooks only when you intentionally need a surface-specific extension point beyond the shared repo files.
- Prefer cross-platform `command` entries first and add shell-specific variants only when a hook genuinely needs them.
...
- Last reviewed: 2026-05-22# AI metadata best practices
