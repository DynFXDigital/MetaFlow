...
function safeExit(message, exitCode = 0) {
    console.log(message);
...
try {
    ...
    if (!Array.isArray(rows)) {
        safeExit('[lint-summary] invalid report format; expected array', 1);
    }
    ...
    console.log(
        `[lint-summary] files=${totals.files} errors=${totals.errors} warnings=${totals.warnings}`,
    );
    ...
    safeExit(`[lint-summary] unable to parse report: ${message}`, 1);
}
