    ...
    test('openCapabilityDetails inline button is accessible on a layer item', async function () {
        ...
        if (actionButtons.length > 0) {
            ...
            if (detailsBtn) {
                ...
                const openTitles = await editorView
                    .getOpenEditorTitles()
                    .catch(() => [] as string[]);
            ...
            try {
                ...
                assert.fail(
                    `openCapabilityDetails command not registered: ${(err as Error).message}`,
                );
            }
        }
    });

    // ── Create README descriptor wizard ──────────────────────────────────────

    test('MetaFlow: Create README Descriptor command opens a multi-step input wizard', async () => {
        const workbench = new Workbench();
        await workbench.executeCommand('MetaFlow: Create README Descriptor');

        // First step: pick or input parent directory
        const input = await InputBox.create(INTERACTION_TIMEOUT);
        assert.ok(input, 'No input dialog appeared after Create README Descriptor command');

        // Cancel at the directory selection step
        await input.cancel();
    });

    test('Create README Descriptor wizard exits cleanly on cancel', async () => {
        const workbench = new Workbench();
        await workbench.executeCommand('MetaFlow: Create README Descriptor');
        ...
        const section = await getSection(sideBar, 'Capabilities');
        assert.ok(section, 'Capabilities section missing after Create README Descriptor cancel');
    });

    test('Create README Descriptor wizard shows capability name prompt after directory selection', async function () {
        this.timeout(30_000);
        const workbench = new Workbench();
        await workbench.executeCommand('MetaFlow: Create README Descriptor');
        ...
        assert.ok(nameInput, 'Expected a capability name input after directory selection');
        if (nameInput) {
            await nameInput.cancel();
        }
    });

    test('Create README Descriptor is accessible from Capabilities view title toolbar', async () => {
        ...
        try {
            ...
            assert.fail(`Create README Descriptor command failed: ${(err as Error).message}`);
        }
    });

    // ── Maintain plugin.json metadata ─────────────────────────────────────────

    test('MetaFlow: Maintain Plugin Manifest (plugin.json) command is registered', async () => {
        const workbench = new Workbench();
        try {
            ...
            await workbench.executeCommand('MetaFlow: Maintain Plugin Manifest (plugin.json)');
            await sleep(500);
        } catch (err) {
