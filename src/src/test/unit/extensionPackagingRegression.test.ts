...
suite('Extension Packaging Regression Guards', () => {
    ...
    test('target support report is contributed for the command palette', () => {
        const packageJsonPath = path.join(EXTENSION_ROOT, 'package.json');
        const packageJson = JSON.parse(
            fs.readFileSync(packageJsonPath, 'utf-8'),
        ) as ExtensionPackageJson;
        ...
        const command = packageJson.contributes?.commands?.find(
            (entry) => entry.command === 'metaflow.openTargetSupportReport',
        );
        assert.ok(command, 'Expected metaflow.openTargetSupportReport command contribution');
    ...
    test('Codex runtime evidence review queue is contributed for the command palette', () => {
        ...
        const command = packageJson.contributes?.commands?.find(
            (entry) => entry.command === 'metaflow.openCodexRuntimeEvidenceReviewQueue',
        );
        assert.ok(
            command,
            'Expected metaflow.openCodexRuntimeEvidenceReviewQueue command contribution',
        );
        assert.strictEqual(command?.icon, '$(list-selection)');
    });

    test('Codex runtime evidence template is contributed for the command palette', () => {
        const packageJsonPath = path.join(EXTENSION_ROOT, 'package.json');
        const packageJson = JSON.parse(
            fs.readFileSync(packageJsonPath, 'utf-8'),
        ) as ExtensionPackageJson;
        ...
        const command = packageJson.contributes?.commands?.find(
            (entry) => entry.command === 'metaflow.openCodexRuntimeEvidenceTemplate',
        );
        assert.ok(command, 'Expected metaflow.openCodexRuntimeEvidenceTemplate command contribution');
        assert.strictEqual(command?.icon, '$(json)');
    });
    ...
    test('package marketplace report is contributed for the command palette', () => {
        ...
        const command = packageJson.contributes?.commands?.find(
            (entry) => entry.command === 'metaflow.openPackageMarketplaceReport',
        );
        assert.ok(command, 'Expected metaflow.openPackageMarketplaceReport command contribution');
