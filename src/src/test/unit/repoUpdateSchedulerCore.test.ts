...
suite('repoUpdateSchedulerCore', () => {
    test('runs startup check and interval check callbacks', () => {
        ...
        assert.strictEqual(startupDelayMs, 1500);
        timeoutCallback?.();
    ...
    test('logScheduled receives the normalized interval value', () => {
        const logged: string[] = [];

        createRepoUpdateSchedulerCore({
            getConfiguredInterval: () => 'weekly',
            onConfigurationChange: () => ({ dispose: () => {} }),
            runScheduledCheck: () => {},
            logScheduled: (interval) => {
                logged.push(interval);
            },
            setIntervalFn: () => 1,
            clearIntervalFn: () => {},
            setTimeoutFn: () => 2,
        ...
        assert.deepStrictEqual(logged, ['weekly']);
    });

    test('dispose does not throw when called a second time', () => {
        const scheduler = createRepoUpdateSchedulerCore({
            getConfiguredInterval: () => 'daily',
            onConfigurationChange: () => ({ dispose: () => {} }),
            runScheduledCheck: () => {},
