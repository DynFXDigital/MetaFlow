...
suite('overlayEngine', () => {
    ...
    suite('resolveLayers — single repo', () => {
        test('normalizes .github-prefixed files to artifact-relative paths', () => {
            ...
            createLayer(repoDir, 'company/core', ['.github/instructions/test.instructions.md']);
        ...
        test('layer with nested directory structure', () => {
            const repoDir = path.join(tmpDir, 'repo');
            createLayer(repoDir, 'deep', ['instructions/sub/deep/file.md', 'instructions/top.md']);
    ...
    suite('resolveLayers — multi repo', () => {
        ...
        test('disabled layer source is skipped', () => {
            ...
            const config: MetaFlowConfig = {
                metadataRepos: [{ id: 'standards', localPath: repo1 }],
                layerSources: [{ repoId: 'standards', path: 'core', enabled: false }],
            };
        ...
        test('invalid repoId is skipped', () => {
            const config: MetaFlowConfig = {
                metadataRepos: [{ id: 'valid', localPath: tmpDir }],
                layerSources: [{ repoId: 'invalid', path: 'core' }],
            };
        ...
        test('disabled repo source is skipped even when layer source is enabled', () => {
            ...
            const config: MetaFlowConfig = {
                metadataRepos: [{ id: 'standards', localPath: repo1, enabled: false }],
                layerSources: [{ repoId: 'standards', path: 'core', enabled: true }],
            };
    ...
    suite('buildEffectiveFileMap', () => {
        test('single layer maps all files', () => {
            const repoDir = path.join(tmpDir, 'repo');
            createLayer(repoDir, 'base', ['instructions/a.md', 'prompts/b.md']);
