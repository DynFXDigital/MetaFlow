    ...
    test('bundled .github artifacts classify into the correct artifact buckets', () => {
        ...
        for (const f of instructionFiles) {
            assert.ok(
                ...
                `Expected instructions artifact under .github/instructions/: ${f}`,
            );
        }
        for (const f of promptFiles) {
            assert.ok(
                f.startsWith('.github/prompts/'),
                `Expected prompts artifact under .github/prompts/: ${f}`,
            );
        }
        for (const f of agentFiles) {
            assert.ok(
                f.startsWith('.github/agents/'),
                `Expected agents artifact under .github/agents/: ${f}`,
            );
        }
        for (const f of skillFiles) {
            assert.ok(
                f.startsWith('.github/skills/'),
                `Expected skills artifact under .github/skills/: ${f}`,
            );
        }
    });
