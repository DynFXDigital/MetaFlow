...
suite('TC-0601: No auto-commit after operations (REQ-0601)', () => {
    ...
    test('materializer module does not import child_process', () => {
        const src = fs.readFileSync(
            path.join(ENGINE_SRC, 'engine', 'materializer.ts'),
            'utf-8'
        );
        assert.strictEqual(
            src.includes('child_process'),
            false,
            'materializer.ts must not import child_process'
        );
    });

    test('overlayEngine module does not import child_process', () => {
        const src = fs.readFileSync(
            path.join(ENGINE_SRC, 'engine', 'overlayEngine.ts'),
            'utf-8'
        );
        assert.strictEqual(
            src.includes('child_process'),
    ...
    test('configLoader module does not import child_process', () => {
        const src = fs.readFileSync(
            path.join(ENGINE_SRC, 'config', 'configLoader.ts'),
            'utf-8'
        );
        assert.strictEqual(
            src.includes('child_process'),
