...
suite('repoSync', () => {
    ...
    test('probeRepoSyncStatus returns repo error when dirty status probe fails', async () => {
        const calls: string[] = [];
        const runner: GitRunner = async (args: string[]) => {
            const key = args.join(' ');
            calls.push(key);
            ...
            if (key === 'status --porcelain') {
                return createResult(false, '', 'fatal: status failed', 'status failed');
            }
            throw new Error(`unexpected git call: ${key}`);
        };
        ...
        assert.deepStrictEqual(calls, ['rev-parse --is-inside-work-tree', 'status --porcelain']);
        assert.strictEqual(status.isGitRepo, true);
        assert.strictEqual(status.hasLocalChanges, false);
        assert.strictEqual(status.hasUpstream, false);
        assert.strictEqual(status.lastError, 'status failed');
    });

    test('probeRepoSyncStatus returns repo error when upstream probe fails', async () => {
        const calls: string[] = [];
        const runner: GitRunner = async (args: string[]) => {
            const key = args.join(' ');
            calls.push(key);
            ...
            if (key === 'rev-parse --abbrev-ref --symbolic-full-name @{u}') {
                return createResult(false, '', 'fatal: no upstream configured', 'no upstream');
            }
            throw new Error(`unexpected git call: ${key}`);
        };
        ...
        assert.strictEqual(status.lastError, 'no upstream');
    });

    test('probeRepoSyncStatus falls back per field when ahead or behind is non-numeric', async () => {
        const responses = new Map<string, GitCommandResult>([
            ['rev-parse --is-inside-work-tree', createResult(true, 'true\n')],
            ['status --porcelain', createResult(true, '')],
            [
                'rev-parse --abbrev-ref --symbolic-full-name @{u}',
                createResult(true, 'origin/main\n'),
            ],
            ['rev-list --left-right --count HEAD...@{u}', createResult(true, 'nope 7x\n')],
        ]);
        ...
        assert.strictEqual(status.hasUpstream, true);
        assert.strictEqual(status.ahead, 0);
        assert.strictEqual(status.behind, 7);
        assert.strictEqual(status.updatesAvailable, true);
        assert.strictEqual(status.pushable, false);
    ...
    test('collectRepoSyncStatuses aggregates by repo id and summarize counts', async () => {
        ...
        const statuses = await collectRepoSyncStatuses(
            workspaceRoot,
            [
                source,
                {
