...
export async function probeRepoSyncStatus(
    ...
    const countsResult = await gitRunner(['rev-list', '--left-right', '--count', 'HEAD...@{u}'], repoRoot);
    if (!countsResult.ok) {
...
export function summarizeRepoSyncStatuses(statuses: Record<string, RepoSyncStatus>): RepoSyncSummary {
    let dirtyCount = 0;
    let updatesCount = 0;
    let pushableCount = 0;
