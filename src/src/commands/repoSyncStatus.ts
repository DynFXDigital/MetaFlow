...
export function parseAheadBehindCounts(rawValue: string): { behindCount: number; aheadCount: number } {
...
export async function checkRepoSyncStatus(
    ...
    try {
        const insideWorkTree = trimOutput((await runner(repoRoot, ['rev-parse', '--is-inside-work-tree'])).stdout);
    ...
    try {
        trackingRef = trimOutput((await runner(repoRoot, ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{upstream}'])).stdout);
        if (!trackingRef) {
    ...
    try {
        ...
        const revList = await runner(repoRoot, ['rev-list', '--left-right', '--count', '@{upstream}...HEAD']);
...
export function mapPullFailureMessage(rawError: string): string {
    ...
    if (lowered.includes('not possible to fast-forward') || lowered.includes('divergent branches')) {
    ...
    if (lowered.includes('could not read from remote repository') || lowered.includes('authentication failed')) {
