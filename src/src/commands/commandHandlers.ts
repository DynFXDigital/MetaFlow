...
const CODEX_RUNTIME_EVIDENCE_REVIEW_QUEUE_IDS = [
    'all',
    'release-ready',
    'missing-evidence',
    'diagnostics',
...
function isCodexRuntimeEvidenceReviewQueueId(
    value: string,
): value is CodexRuntimeEvidenceReviewQueueId {
    return CODEX_RUNTIME_EVIDENCE_REVIEW_QUEUE_IDS.includes(
        value as CodexRuntimeEvidenceReviewQueueId,
    );
}

function formatCodexRuntimeEvidenceConcepts(concepts: TargetCapabilityConcept[]): string {
    return concepts.length > 0 ? concepts.join(', ') : 'none';
}

function uniqueCodexRuntimeEvidenceConcepts(
    concepts: TargetCapabilityConcept[],
): TargetCapabilityConcept[] {
...
export function buildCodexRuntimeEvidenceReviewQueueDocument(
    ...
    const generatedBy =
        options?.generatedBy ?? 'metaflow extension codex-runtime-evidence-review-queue';
    ...
    const gateConditions: CodexRuntimeEvidenceGateCondition[] =
        queue === 'all'
            ...
            : queue === 'release-ready'
              ? supportDocument.runtimeEvidenceReadinessSummary.checkedConditions
              : [queue];
    const queueConcepts =
        queue === 'all'
            ? supportDocument.runtimeEvidenceChecklist.map((item) => item.concept)
            : queue === 'release-ready'
              ? supportDocument.runtimeEvidenceReadinessSummary.blockingConditions.flatMap(
                    (condition) =>
                        supportDocument.runtimeEvidenceGateSummary[condition]?.concepts ?? [],
                )
              : supportDocument.runtimeEvidenceGateSummary[queue]?.concepts ?? [];
    const concepts = uniqueCodexRuntimeEvidenceConcepts(queueConcepts);
    const conceptSet = new Set(concepts);
    const actionPlan = supportDocument.runtimeEvidenceActionPlan.filter(
        (item) =>
            queue === 'all' ||
            (queue === 'release-ready' &&
                supportDocument.runtimeEvidenceReadinessSummary.blockingConditions.includes(
                    item.condition,
                )) ||
    ...
    const checklist =
        queue === 'all'
            ? supportDocument.runtimeEvidenceChecklist
            : supportDocument.runtimeEvidenceChecklist.filter((item) =>
    ...
    const lines = [
        ...
        `Codex adapter version \`${supportDocument.adapterVersion}\`.`,
        `Queue \`${queue}\`.`,
        '',
        ...
        '## Readiness',
        '',
        `Release-ready preset: ${supportDocument.runtimeEvidenceReadinessSummary.ready ? 'ready' : 'blocked'}.`,
        `Blocking gates: ${supportDocument.runtimeEvidenceReadinessSummary.blockingConditions.length > 0 ? supportDocument.runtimeEvidenceReadinessSummary.blockingConditions.join(', ') : 'none'}.`,
        '',
        '## Queue Summary',
        '',
        ...
        ...gateConditions.map((condition) => {
            const gate = supportDocument.runtimeEvidenceGateSummary[condition];
            return `| ${condition} | ${gate.triggered ? 'yes' : 'no'} | ${gate.count} | ${formatCodexRuntimeEvidenceConcepts(gate.concepts)} |`;
        }),
        '',
        '## Coverage Queues',
        '',
        `- Missing evidence: ${formatCodexRuntimeEvidenceConcepts(supportDocument.runtimeEvidenceCoverageSummary.conceptsByStatus.missing)}`,
        `- Evidence without diagnostics: ${formatCodexRuntimeEvidenceConcepts(supportDocument.runtimeEvidenceCoverageSummary.conceptsWithEvidenceWithoutDiagnosticRecords)}`,
        `- Evidence with diagnostics: ${formatCodexRuntimeEvidenceConcepts(supportDocument.runtimeEvidenceCoverageSummary.conceptsWithEvidenceWithDiagnosticRecords)}`,
        `- Evidence with error diagnostics: ${formatCodexRuntimeEvidenceConcepts(supportDocument.runtimeEvidenceCoverageSummary.conceptsWithErrorRecords)}`,
        '',
        '## Action Items',
        '',
    ];
    ...
    if (actionPlan.length === 0) {
        ...
        for (const action of actionPlan) {
            lines.push(
                ...
                `  Concepts: ${formatCodexRuntimeEvidenceConcepts(action.concepts)}`,
            );
    ...
    return {
        generatedBy,
        generatedAt,
        adapterVersion: supportDocument.adapterVersion,
        queue,
        concepts,
        content: `${lines.join('\n')}\n`,
    };
...
export function buildCodexRuntimeEvidenceReviewQueueDocumentForExtension(
    ...
    return buildCodexRuntimeEvidenceReviewQueueDocument(
        buildCodexSupportBoundariesDocumentForExtension(),
        queue,
        {
            generatedBy: 'metaflow extension codex-runtime-evidence-review-queue',
        },
    );
}

export function buildCodexRuntimeEvidenceReviewQueueDocumentForWorkspace(
    config: MetaFlowConfig,
    workspaceRoot: string,
    queue: CodexRuntimeEvidenceReviewQueueId = 'all',
): CodexRuntimeEvidenceReviewQueueDocument {
...
export function buildPackageMarketplaceReportForExtension(
    config: MetaFlowConfig,
    workspaceRoot: string,
): PackageMarketplaceReport & { content: string } {
...
export function registerCommands(
    ...
    // ── metaflow.openCodexSupportBoundaries ───────────────────────
    context.subscriptions.push(
        vscode.commands.registerCommand('metaflow.openCodexSupportBoundaries', async () => {
            try {
                ...
                logInfo(
                    `${document.runtimeOnlyCount} Codex runtime-only support row(s). Repository projection cannot make these operational without operator or harness evidence.`,
                );
                ...
                await vscode.window.showTextDocument(doc, { preview: false });
                vscode.window.showInformationMessage(
                    'MetaFlow: Opened Codex support boundaries. Review runtime-only and unprojectable surfaces before relying on generated files.',
                );
            } catch (err: unknown) {
                const message = err instanceof Error ? err.message : String(err);
                showOutputChannel();
                ...
                try {
                    ...
                    let queue =
                        typeof arg === 'string' && arg.trim().length > 0
                            ...
                            : undefined;
                    if (!queue) {
                        queue = await vscode.window.showQuickPick(
                            [...CODEX_RUNTIME_EVIDENCE_REVIEW_QUEUE_IDS],
                            {
                                title: 'Open Codex Runtime Evidence Review Queue',
                                placeHolder: 'Select a runtime evidence queue',
                            },
                        );
                    }
                    if (!queue) {
                    ...
                    if (!isCodexRuntimeEvidenceReviewQueueId(queue)) {
                        vscode.window.showErrorMessage(
                            `MetaFlow: Unknown Codex runtime evidence review queue "${queue}".`,
                        );
                        return;
                    }
                    const document = buildCodexRuntimeEvidenceReviewQueueDocument(
                        supportDocument,
                        queue,
                        {
                            generatedBy:
                    ...
                    showOutputChannel();
                    logInfo('=== Codex Runtime Evidence Review Queue ===');
                    logInfo(`Queue: ${queue}`);
                    logInfo(`Concepts: ${document.concepts.length}`);
                    ...
                    await vscode.window.showTextDocument(doc, { preview: false });
                    vscode.window.showInformationMessage(
                        'MetaFlow: Opened Codex runtime evidence review queue. Use it to triage evidence before claiming runtime support.',
                    );
                } catch (err: unknown) {
                    const message = err instanceof Error ? err.message : String(err);
                    showOutputChannel();
