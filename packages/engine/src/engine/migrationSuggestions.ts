...
import { LayerContent } from './types';
...
export interface MigrationSuggestionsReport {
    generatedBy: string;
    managed: false;
    writesFiles: false;
    summary: {
...
export function buildMigrationSuggestionsReport(layers: LayerContent[]): MigrationSuggestionsReport {
    const sources = collectMigrationSources(layers);
    const canonicalPaths = new Set(
        sources.map((source) => normalizeRelativePath(source.sourcePath)),
    );
...
export function formatMigrationSuggestionsReport(report: MigrationSuggestionsReport): string {
    const lines = [
