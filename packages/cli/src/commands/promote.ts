...
import { checkAllDrift, loadManagedState, loadConfig, stripProvenanceHeader } from '@metaflow/engine';
...
/**
 * Automated promotion: copies drifted materialized files back to the
...
export function promoteAuto(
    ...
    if (tracked.length === 0) {
        return { branch: '', filesPromoted: [], committed: false, error: 'No managed files to check.' };
    }
    ...
    if (drifted.length === 0) {
        return { branch: '', filesPromoted: [], committed: false, error: 'No drifted files to promote.' };
    }
    ...
    const repoLocalPath = configResult.config.metadataRepo?.localPath;
    if (!repoLocalPath) {
        return { branch: '', filesPromoted: [], committed: false, error: 'Cannot determine metadata repo path.' };
    }
    const repoPath = path.resolve(workspaceRoot, repoLocalPath);

    if (!isGitRepo(repoPath)) {
        return { branch: '', filesPromoted: [], committed: false, error: `Metadata repo at ${repoPath} is not a git repository.` };
    }
    ...
    try {
        ...
        for (const d of drifted) {
            ...
            const materializedPath = path.join(workspaceRoot, '.github', d.relativePath);
            const materializedContent = fs.readFileSync(materializedPath, 'utf-8');
            const cleanContent = stripProvenanceHeader(materializedContent);

            const targetPath = path.join(repoPath, sourceLayer, d.relativePath);
        ...
        if (filesPromoted.length === 0) {
            return { branch: branchName, filesPromoted: [], committed: false, error: 'No files could be promoted (unknown source layers).' };
        }

        // Stage and commit
        git(repoPath, 'add .');
        const commitMessage = options.message ?? `chore: promote ${filesPromoted.length} drifted file(s)`;
...
export function registerPromoteCommand(program: Command): void {
    program
        .command('promote')
        .description('Detect locally modified materialized files, or auto-promote them')
        .option('--auto', 'Create branch and commit drifted files to metadata repo')
        .option('--branch <name>', 'Branch name for --auto promotion')
        .option('--layer <path>', 'Target layer path for --auto promotion (overrides auto-detection)')
