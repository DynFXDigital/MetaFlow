...
interface RuntimeEvidenceGuideConcept {
    concept: string;
...
interface RuntimeEvidenceGuideReport {
    schemaVersion: 'metaflow.runtimeEvidenceGuide/v1';
    generatedBy: string;
    generatedAt: string;
    adapterVersion: string;
    target: 'codex';
    concepts: RuntimeEvidenceGuideConcept[];
    content: string;
}
...
function buildRuntimeEvidenceGuideReport(
    document: CodexSupportBoundariesDocument,
    concepts: string[],
): RuntimeEvidenceGuideReport {
    const requestedConcepts = new Set(concepts);
    const guideConcepts = document.runtimeEvidenceChecklist
        .filter((item) => requestedConcepts.has(item.concept))
        .map((item) => {
            const conceptSlug = toKebabCase(item.concept);
            return {
    ...
    const lines = [
        ...
        '',
        'Generated by `metaflow codex-support-boundaries --runtime-evidence-guide`.',
        `Generated at \`${document.generatedAt}\`.`,
        `Codex adapter version \`${document.adapterVersion}\`.`,
        '',
    ...
    return {
        schemaVersion: 'metaflow.runtimeEvidenceGuide/v1',
        generatedBy: 'metaflow codex-support-boundaries --runtime-evidence-guide',
        generatedAt: document.generatedAt,
        adapterVersion: document.adapterVersion,
        target: 'codex',
    ...
    program
        ...
        .action((options: CodexSupportBoundariesOptions) => {
            ...
            if (options.runtimeEvidenceGuide) {
                const guideReport = buildRuntimeEvidenceGuideReport(
                    document,
                    runtimeEvidenceConcepts,
                );
