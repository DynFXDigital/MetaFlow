...
describe('CLI: status', () => {
    ...
    it('should include canonical capability manifest details in status output', async () => {
        ...
        assert.ok(textResult.stdout.includes('Target Capability Support: 68'));
        assert.ok(
            textResult.stdout.includes(
                'codex (codex-v0.1): partial=11, runtime-only=20, supported=3',
            ),
        );
        assert.ok(
            textResult.stdout.includes(
                'Runtime-only support boundaries: 35 rows require operator or harness evidence; codex=20 see docs/CODEX-SUPPORT.md; github-copilot=15 see README.md.',
            ),
        );
        ...
        assert.strictEqual(data.targetCapabilitySupport.entries, 68);
        ...
        assert.strictEqual(codexTargetSupport.counts['runtime-only'], 20);
...
describe('CLI: target-support', () => {
    it('prints filtered target support rows without requiring workspace config', async () => {
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('Target Support Matrix: 20'));
        assert.ok(
            result.stdout.includes(
                'Runtime-only support boundaries: 20 rows require operator or harness evidence; codex=20 see docs/CODEX-SUPPORT.md.',
            ),
        );
...
describe('CLI: codex-support-boundaries', () => {
    ...
    it('prints Codex support boundaries as JSON', async () => {
        ...
        assert.strictEqual(data.runtimeOnlyCount, 20);
        assert.ok(
    ...
    it('writes Codex support boundaries JSON to an explicit output path', async () => {
        ...
        assert.strictEqual(data.generatedBy, 'metaflow codex-support-boundaries');
        assert.strictEqual(data.runtimeOnlyCount, 20);
        assert.strictEqual(data.runtimeOnlyRows.length, 20);
...
describe('CLI: validate', () => {
    ...
    it('should pass validation after clean apply', async () => {
        ...
        assert.ok(result.stdout.includes('Target Capability Support: 68'));
        assert.ok(
            result.stdout.includes(
                'Runtime-only support boundaries: 35 rows require operator or harness evidence',
            ),
        );
    });
    ...
    it('validate --json returns structured result', async () => {
        ...
        assert.ok(
            ...
            (entry: { target: string; counts: Record<string, number> }) =>
                entry.target === 'codex' && entry.counts['runtime-only'] === 20,
            ),
        );
        assert.strictEqual(data.targetCapabilitySupport.supportReference.runtimeOnlyCount, 35);
    });
