...
describe('CLI: status', () => {
    ...
    it('should include canonical capability manifest details in status output', async () => {
        ...
        assert.ok(textResult.stdout.includes('Target Capability Support: 98'));
        assert.ok(
            textResult.stdout.includes(
                'codex (codex-v0.1): partial=12, runtime-only=34, supported=3',
            ),
        );
        assert.ok(
            textResult.stdout.includes(
                'Runtime-only support boundaries: 50 rows require operator or harness evidence; codex=34 see docs/CODEX-SUPPORT.md; github-copilot=16 see README.md.',
            ),
        );
        ...
        assert.strictEqual(data.targetCapabilitySupport.entries, 98);
        ...
        assert.strictEqual(codexTargetSupport.counts['runtime-only'], 34);
...
describe('CLI: target-support', () => {
    it('prints filtered target support rows without requiring workspace config', async () => {
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('Target Support Matrix: 34'));
        assert.ok(
            result.stdout.includes(
                'Runtime-only support boundaries: 34 rows require operator or harness evidence; codex=34 see docs/CODEX-SUPPORT.md.',
            ),
        );
        ...
        assert.ok(result.stdout.includes('codex/nonInteractiveRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex/sdkRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex/appServerRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex/ideExtensionRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex/windowsPlatformRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex/linuxPlatformRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex/macosPlatformRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex/localEnvironmentRuntime: runtime-only'));
    ...
    it('prints local environment runtime boundaries in target-support output', async () => {
        const result = await runCli([
            'target-support',
            '--target',
            'codex',
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('codex/localEnvironmentRuntime: runtime-only'));
        assert.ok(result.stdout.includes('Codex app local environments'));
        assert.ok(result.stdout.includes('worktree setup scripts'));
        assert.ok(result.stdout.includes('integrated-terminal actions'));
        assert.ok(result.stdout.includes('evidence: RUN-087'));
    });

    it('prints non-interactive runtime boundaries in target-support output', async () => {
        const result = await runCli([
            'target-support',
            '--target',
            'codex',
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('codex/nonInteractiveRuntime: runtime-only'));
        assert.ok(result.stdout.includes('codex exec'));
        assert.ok(result.stdout.includes('CODEX_API_KEY'));
        assert.ok(result.stdout.includes('output-schema'));
        assert.ok(result.stdout.includes('evidence: RUN-088'));
    });

    it('prints SDK runtime boundaries in target-support output', async () => {
        const result = await runCli([
            'target-support',
            '--target',
            'codex',
        ...
        assert.ok(result.stdout.includes('@openai/codex-sdk'));
        assert.ok(result.stdout.includes('openai-codex Python package'));
        assert.ok(result.stdout.includes('SDK sandbox presets'));
        assert.ok(result.stdout.includes('evidence: RUN-089'));
    });

    it('prints app-server runtime boundaries in target-support output', async () => {
        const result = await runCli([
            'target-support',
            '--target',
            'codex',
        ...
        assert.ok(result.stdout.includes('Codex app-server JSON-RPC 2.0 runtime'));
        assert.ok(result.stdout.includes('WebSocket app-server transport'));
        assert.ok(result.stdout.includes('initialize / initialized handshake'));
        assert.ok(result.stdout.includes('evidence: RUN-090'));
    });

    it('prints IDE extension runtime boundaries in target-support output', async () => {
        const result = await runCli([
            'target-support',
            '--target',
            'codex',
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('codex/ideExtensionRuntime: runtime-only'));
        assert.ok(result.stdout.includes('Codex IDE extension'));
        assert.ok(result.stdout.includes('open files context'));
        assert.ok(result.stdout.includes('selected text range context'));
        assert.ok(result.stdout.includes('cannot install the extension'));
        assert.ok(result.stdout.includes('evidence: RUN-091'));
    });
...
describe('CLI: codex-support-boundaries', () => {
    ...
    it('prints Codex support boundaries as JSON', async () => {
        ...
        assert.ok(
            data.notAchievableByRepositoryProjection.some((item: string) =>
                item.includes('Selecting native Windows sandbox implementation'),
            ),
        );
        assert.ok(
            data.notAchievableByRepositoryProjection.some((item: string) =>
                item.includes('Installing bubblewrap'),
            ),
        );
        assert.ok(
            data.notAchievableByRepositoryProjection.some((item: string) =>
    ...
    it('prints a runtime-complete runtime evidence review queue with workspace evidence', async () => {
        ws = createTestWorkspace({
            config: {
                metadataRepo: { localPath: '.ai/ai-metadata' },
                layers: ['company/core'],
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('Queue `runtime-complete`.'));
        assert.ok(result.stdout.includes('| missing-evidence | yes |'));
        assert.ok(result.stdout.includes('| partial | yes | 1 | reviewRuntime |'));
        assert.ok(
            result.stdout.includes(
    ...
    it('prints a completion-readiness runtime evidence review queue with workspace evidence', async () => {
        ws = createTestWorkspace({
            config: {
                metadataRepo: { localPath: '.ai/ai-metadata' },
                layers: ['company/core'],
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('Queue `completion-readiness`.'));
        assert.ok(result.stdout.includes('## Completion Readiness Queues'));
        assert.ok(result.stdout.includes('- External-authority bound: reviewRuntime'));
        assert.ok(result.stdout.includes('- Hosted/network bound: reviewRuntime'));
        assert.ok(result.stdout.includes('- App/platform bound: reviewRuntime'));
        assert.ok(
            result.stdout.includes(
                '- complete-partial-runtime-evidence (partial, blocking): 1 runtime-only concept(s) are covered by partial evidence',
            ),
    ...
    it('prints structured JSON for completion-readiness runtime evidence review queues', async () => {
        ws = createTestWorkspace({
            config: {
                metadataRepo: { localPath: '.ai/ai-metadata' },
                layers: ['company/core'],
        ...
        const currentEnvironmentResult = await runCli([
            'codex-support-boundaries',
            '--runtime-evidence-review-queue',
            'completion-readiness-current-environment',
            '--json',
        ...
        assert.strictEqual(currentEnvironmentResult.exitCode, 0);
        const allData = JSON.parse(allResult.stdout);
        const currentEnvironmentData = JSON.parse(currentEnvironmentResult.stdout);
        assert.strictEqual(allData.queue, 'completion-readiness');
        assert.strictEqual(allData.runtimeEvidenceCompletionReadinessSummary.partialConcepts, 2);
        assert.deepStrictEqual(
            allData.completionReadinessItems.map(
                (item: { concept: string }) => item.concept,
            ),
            ['modelProviderRuntime', 'reviewRuntime'],
        );
        assert.strictEqual(
            currentEnvironmentData.queue,
            'completion-readiness-current-environment',
        );
        assert.strictEqual(
            currentEnvironmentData.runtimeEvidenceCompletionReadinessSummary.partialConcepts,
            2,
        );
        assert.deepStrictEqual(
            currentEnvironmentData.completionReadinessItems.map(
                (item: { concept: string }) => item.concept,
            ),
            ['modelProviderRuntime'],
        );
        assert.ok(currentEnvironmentData.content.includes('| modelProviderRuntime | partial |'));
        assert.ok(!currentEnvironmentData.content.includes('| reviewRuntime | partial |'));
    });
    ...
    it('writes Codex support boundaries JSON to an explicit output path', async () => {
        ...
        assert.strictEqual(data.generatedBy, 'metaflow codex-support-boundaries');
        assert.strictEqual(data.runtimeOnlyCount, 34);
        assert.strictEqual(data.runtimeOnlyRows.length, 34);
...
describe('CLI: validate', () => {
    ...
    it('should pass validation after clean apply', async () => {
        ...
        assert.ok(result.stdout.includes('Target Capability Support: 98'));
        assert.ok(
            result.stdout.includes(
                'Runtime-only support boundaries: 50 rows require operator or harness evidence',
            ),
        );
    });
        ...
        assert.ok(
            data.targetCapabilitySupport.targets.some(
                (entry: { target: string; counts: Record<string, number> }) =>
                    entry.target === 'codex' && entry.counts['runtime-only'] === 34,
            ),
        );
        assert.strictEqual(data.targetCapabilitySupport.supportReference.runtimeOnlyCount, 50);
    });
