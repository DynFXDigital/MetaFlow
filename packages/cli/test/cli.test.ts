...
describe('CLI: status', () => {
    ...
    it('should include canonical capability manifest details in status output', async () => {
        ...
        assert.ok(textResult.stdout.includes('Target Capability Support: 88'));
        assert.ok(
            textResult.stdout.includes(
                'codex (codex-v0.1): partial=12, runtime-only=29, supported=3',
            ),
        );
        assert.ok(
            textResult.stdout.includes(
                'Runtime-only support boundaries: 45 rows require operator or harness evidence; codex=29 see docs/CODEX-SUPPORT.md; github-copilot=16 see README.md.',
            ),
        );
        ...
        assert.strictEqual(data.targetCapabilitySupport.entries, 88);
        ...
        assert.strictEqual(codexTargetSupport.counts['runtime-only'], 29);
...
describe('CLI: target-support', () => {
    it('prints filtered target support rows without requiring workspace config', async () => {
        ...
        assert.strictEqual(result.exitCode, 0);
        assert.ok(result.stdout.includes('Target Support Matrix: 29'));
        assert.ok(
            result.stdout.includes(
                'Runtime-only support boundaries: 29 rows require operator or harness evidence; codex=29 see docs/CODEX-SUPPORT.md.',
            ),
        );
...
describe('CLI: codex-support-boundaries', () => {
    ...
    it('prints Codex support boundaries as JSON', async () => {
        ...
        assert.strictEqual(data.runtimeOnlyCount, 29);
        assert.ok(
    ...
    it('writes Codex support boundaries JSON to an explicit output path', async () => {
        ...
        assert.strictEqual(data.generatedBy, 'metaflow codex-support-boundaries');
        assert.strictEqual(data.runtimeOnlyCount, 29);
        assert.strictEqual(data.runtimeOnlyRows.length, 29);
...
describe('CLI: validate', () => {
    ...
    it('should pass validation after clean apply', async () => {
        ...
        assert.ok(result.stdout.includes('Target Capability Support: 88'));
        assert.ok(
            result.stdout.includes(
                'Runtime-only support boundaries: 45 rows require operator or harness evidence',
            ),
        );
    });
        ...
        assert.ok(
            data.targetCapabilitySupport.targets.some(
                (entry: { target: string; counts: Record<string, number> }) =>
                    entry.target === 'codex' && entry.counts['runtime-only'] === 29,
            ),
        );
        assert.strictEqual(data.targetCapabilitySupport.supportReference.runtimeOnlyCount, 45);
    });
