    ...
    it('rejects invalid or ambiguous runtime evidence review queue options', async () => {
        ...
        assert.ok(
            unknown.stderr.includes(
                '--runtime-evidence-review-queue must be one of: all, release-ready, runtime-complete, missing-evidence, diagnostics, error-diagnostics, failed, not-run, partial, expired-evidence, stale-adapter-version, waived',
            ),
        );
