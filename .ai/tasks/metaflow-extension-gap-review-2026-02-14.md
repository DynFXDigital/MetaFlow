...
- [ ] Decide and implement single-repo layer toggling behavior (or explicitly mark unsupported).
...
- [ ] Add integration tests that assert workspace settings updates after `metaflow.apply`.
- [ ] Add integration tests that assert settings cleanup after `metaflow.clean`.
...
- [ ] Add a first-run command flow: `initConfig` -> `preview` -> `apply` -> `status` with concise success/failure summaries.
- [ ] Make preview output explicitly list pending `add/update/skip/remove` per file in a user-scannable format.
- [ ] Add clear conflict resolution guidance when drift blocks apply/clean (`keep local`, `take shared`, `merge manually`).
- [ ] Add selective apply scope support (artifact type and/or path filters) for safer incremental rollout.

### Milestone 2 — Governance UX (Next)

- [ ] Add a consolidated status surface (command output and/or view) showing: last apply time, drift count, managed count, active profile, and settings injection state.
- [ ] Standardize provenance visibility by surfacing source repo/layer/path for materialized files in preview/status output.
...
- [ ] Expand `metaflow.promote` from detection-only toward guided branch/commit/PR scaffolding (no auto-push).
...
- [ ] Add integration tests covering end-to-end promote preparation workflow from drift detection to staged artifacts.
