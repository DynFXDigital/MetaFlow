...
- Canonical runtime evidence records attach target-level harness proof, partial proof, failed runs, `not-run` records, or waivers to target capability concepts such as `issuePrOperation`, `reviewRuntime`, `cloudEnvironmentRuntime`, and `permissionRuntime`. MetaFlow uses `.metaflow/runtime-evidence/*.json` records to make runtime-only support claims reviewable without treating repository metadata as the runtime authority.
...
- The CLI `codex-support-boundaries` command prints or writes the same Codex support boundary report exposed by the VS Code command palette, including file-backed, runtime-only, concept-keyed runtime evidence checklist, workspace runtime evidence records when present, and not-technically-projectable surfaces.
