...
- Target adapters that mark authority-sensitive concepts such as agents, agent runtime, automation runtime, authentication runtime, permission runtime, enterprise policy runtime, review runtime, remote connection runtime, Chronicle runtime, Record & Replay runtime, import runtime, project config, worktree include, MCP servers, hooks, tools, execution, memory, handoff, issue/PR operation, or evaluation as `managed` must declare adapter-level `requiredPolicyGrants`; otherwise MetaFlow treats those projections as candidate output.
