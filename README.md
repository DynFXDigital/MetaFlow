...
- Codex support-boundary reports include a runtime evidence gate summary for `missing-evidence`, `diagnostics`, `error-diagnostics`, `failed`, and `not-run` checks. The CLI `--fail-on` option uses the same summary that appears in Markdown and JSON output, so release automation can inspect the report artifact and the command exit result from one contract.
