...
- Canonical runtime evidence records attach target-level harness proof, partial proof, failed runs, `not-run` records, or waivers to target capability concepts such as `issuePrOperation`, `reviewRuntime`, `cloudEnvironmentRuntime`, and `permissionRuntime`. MetaFlow uses `.metaflow/runtime-evidence/*.json` records to make runtime-only support claims reviewable without treating repository metadata as the runtime authority. Local structured artifact refs for reports, logs, screenshots, traces, recordings, and files are checked relative to the metadata layer so stale proof paths surface as diagnostics; optional `sha256` digests detect changed local artifacts.
