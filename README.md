...
- Codex support-boundary reports include a runtime evidence readiness summary for the `release-ready` preset plus gate details for `missing-evidence`, `diagnostics`, `error-diagnostics`, `failed`, and `not-run` checks. The CLI `--fail-on` option uses the same gate semantics that appear in Markdown and JSON output, so release automation can inspect the report artifact and the command exit result from one contract.
