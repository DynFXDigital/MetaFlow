...
- The VS Code command `MetaFlow: Open Codex Runtime Evidence Template` opens review-only JSON templates for selected runtime-only Codex concepts from the runtime evidence checklist or a supplied runtime evidence review queue, without creating proof or writing canonical evidence records.
- The VS Code command `MetaFlow: Save Codex Runtime Evidence Template Records` writes selected runtime evidence scaffold files under `.metaflow/runtime-evidence` for selected concepts or a supplied runtime evidence review queue only after explicit confirmation, and existing records require overwrite confirmation before replacement.
