...
- The CLI `codex-support-boundaries` command prints or writes the same Codex support boundary report exposed by the VS Code command palette, including file-backed, runtime-only, runtime evidence coverage summary, runtime evidence clean-versus-diagnostic counts, runtime evidence diagnostic severity counts, concept-keyed runtime evidence checklist, workspace runtime evidence records when present, and not-technically-projectable surfaces.
