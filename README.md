...
- Codex command rules (`.codex/rules/*.rules`) are file-backed policy surfaces. MetaFlow can guard and report those files, but Codex trust, startup loading, shell-wrapper splitting, admin requirements, and command execution decisions remain harness runtime behavior.
