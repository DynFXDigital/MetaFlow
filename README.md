...
- The CLI `codex-support-boundaries` command prints or writes the same Codex support boundary report exposed by the VS Code command palette, including generator metadata, generated timestamp, Codex adapter version, file-backed, runtime-only, runtime evidence readiness summary, runtime evidence action plan with concept details, runtime evidence coverage summary, runtime evidence clean-versus-diagnostic counts, runtime evidence diagnostic severity counts, expired evidence counts, concept-keyed runtime evidence checklist, workspace runtime evidence records when present, and not-technically-projectable surfaces. Its `--projection-boundary-review` mode emits a focused projection-boundary artifact for retaining file-backed, runtime-only, unsupported, not-achievable, and expected-evidence boundaries. Its `--runtime-evidence-review-queue` mode emits focused artifacts for all runtime-only concepts, release-ready blockers, individual gate queues, expired evidence, or waived evidence. Its `--fail-on` gate can exit nonzero for missing evidence, any diagnostics, error diagnostics, failed evidence, or not-run evidence while still emitting the report; `release-ready` and `all` presets cover common release and CI usage.
...
- The VS Code command `MetaFlow: Open Codex Runtime Evidence Review Queue` opens a focused triage document for all runtime-only concepts, release-ready blockers, missing evidence, diagnostics, error diagnostics, expired evidence, failed evidence, not-run evidence, or waived evidence.
