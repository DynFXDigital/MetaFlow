...
- Completion-readiness category queues are available as `completion-readiness-current-environment`, `completion-readiness-external-authority`, `completion-readiness-hosted-network`, and `completion-readiness-app-platform`. They filter partial runtime-complete action items and checklist rows to the selected proof category.
