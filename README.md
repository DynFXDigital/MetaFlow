...
- Codex support-boundary reports include a runtime evidence readiness summary for the `release-ready` preset, a runtime evidence waiver summary for waived and repository-projection-impossible surfaces, a technical impossibility summary for features repository metadata cannot generate, a runtime evidence completeness summary for release-ready versus runtime-complete status, a runtime evidence completion blocker summary for partial concepts, a runtime evidence completion readiness summary for current-environment, external-authority, hosted/network, and app/platform proof needs, a release-ready runtime evidence action plan, a runtime-complete completion action plan, and gate details for `missing-evidence`, `diagnostics`, `error-diagnostics`, `failed`, `not-run`, and `partial` checks. Release-ready means the configured gates have no blockers; it can still include `partial` or `waived` evidence and does not mean every native Codex runtime surface has been fully proven. The `runtime-complete` fail-on preset adds partial evidence to the release-ready gates so CI and operators can distinguish releasable boundary documentation from fully proven native runtime behavior. Each action plan item carries concept-level details for native surfaces, expected proof, authority implications, matching runtime evidence record IDs, and current runtime evidence limitations. The completion action plan lists partial runtime-only concepts that need stronger harness-native proof before `runtime-complete` can pass. The CLI `--fail-on` option uses the same gate semantics that appear in Markdown and JSON output, so release automation can inspect the report artifact and the command exit result from one contract. The CLI `--projection-boundary-review` mode emits focused repository projection boundary documents for Codex file-backed, runtime-only, unsupported, and not-achievable surfaces; `--runtime-evidence-review-queue` emits focused review-only triage documents for all runtime-only concepts, release-ready blockers, runtime-complete completion work, completion-readiness category triage, individual gate queues, partial evidence, expired evidence, stale adapter version evidence, or waived runtime evidence; `--runtime-evidence-guide` emits a review-only collection guide for selected runtime-only concepts; `--runtime-evidence-template` emits a JSON bundle of suggested `.metaflow/runtime-evidence/*.json` records from the release-ready action plan, the completion action plan when release-ready has no blockers, selected runtime-only concepts when `--runtime-evidence-concept` is supplied, or review-queue concepts when `--runtime-evidence-template-queue` is supplied; and `--runtime-evidence-template-dir` writes those fill-in records as individual scaffold files. `--runtime-evidence-concept` scopes guide and template output while evidence is collected incrementally, and `--runtime-evidence-template-queue` scopes template output to a runtime evidence review queue such as a completion-readiness category, without creating runtime proof. Completion-readiness queue-scoped template JSON also includes the readiness summary and filtered completion-readiness items for scaffold routing automation. The VS Code `MetaFlow: Open Codex Projection Boundary Review` command opens the focused projection-boundary document from the same support-boundary data. `MetaFlow: Open Codex Runtime Evidence Review Queue` opens focused triage documents for the same queues. `MetaFlow: Open Codex Runtime Evidence Template` opens a template scope picker for suggested runtime evidence actions, manual runtime-only concept selection, or runtime evidence review queues, then opens the review-only JSON template bundle as an unsaved document without writing canonical evidence records. `MetaFlow: Save Codex Runtime Evidence Template Records` uses the same template scope picker and writes selected scaffold files under `.metaflow/runtime-evidence` only after explicit confirmation and keeps overwrite protection for existing records.
...
- The CLI `codex-support-boundaries` command prints or writes the same Codex support boundary report exposed by the VS Code command palette, including generator metadata, generated timestamp, Codex adapter version, file-backed, runtime-only, runtime evidence readiness summary, runtime evidence waiver summary, technical impossibility summary, runtime evidence completeness summary, runtime evidence completion blocker summary, runtime evidence completion readiness summary, release-ready runtime evidence action plan with concept details and current limitations, runtime-complete completion action plan, runtime evidence coverage summary, runtime evidence clean-versus-diagnostic counts, runtime evidence diagnostic severity counts, expired evidence counts, stale adapter version evidence counts, concept-keyed runtime evidence checklist, workspace runtime evidence records when present, and not-technically-projectable surfaces. Its `--projection-boundary-review` mode emits a focused projection-boundary artifact for retaining file-backed, runtime-only, unsupported, technical-impossibility category counts, not-achievable, and expected-evidence boundaries. Its `--runtime-evidence-review-queue` mode emits focused artifacts for all runtime-only concepts, release-ready blockers, runtime-complete completion work, completion-readiness category triage, individual gate queues, partial evidence, expired evidence, stale adapter version evidence, or waived evidence. Its `--fail-on` gate can exit nonzero for missing evidence, any diagnostics, error diagnostics, failed evidence, not-run evidence, or partial evidence while still emitting the report; `release-ready`, `runtime-complete`, and `all` presets cover common release, completion, and CI usage.
- Completion-readiness category queues are available as `completion-readiness-current-environment`, `completion-readiness-external-authority`, `completion-readiness-hosted-network`, and `completion-readiness-app-platform`. They filter partial runtime-complete action items and checklist rows to the selected proof category, and their JSON documents include the completion-readiness summary plus queue-filtered completion-readiness items for automation.
...
- The VS Code command `MetaFlow: Open Codex Support Boundaries` opens a generated Markdown report that separates file-backed Codex projections from runtime-only and not-technically-projectable surfaces, including workspace runtime evidence records when a MetaFlow config is loaded.
- The VS Code command `MetaFlow: Open Codex Projection Boundary Review` opens a focused Markdown document for Codex file-backed, runtime-only, unsupported, not-achievable, and expected-evidence boundaries.
- The VS Code command `MetaFlow: Open Codex Runtime Evidence Review Queue` opens a focused triage document for all runtime-only concepts, release-ready blockers, runtime-complete completion work, completion-readiness category triage, missing evidence, diagnostics, error diagnostics, partial evidence, expired evidence, stale adapter version evidence, failed evidence, not-run evidence, or waived evidence.
- The VS Code command `MetaFlow: Open Codex Runtime Evidence Guide` opens focused runtime evidence collection guidance for a selected runtime-only Codex concept, including matching workspace runtime evidence record IDs when a MetaFlow config is loaded, without creating proof or writing canonical evidence records.
- The VS Code command `MetaFlow: Open Codex Runtime Evidence Template` opens review-only JSON templates through a template scope picker for suggested runtime evidence actions, manual runtime-only concept selection, or runtime evidence review queues, without creating proof or writing canonical evidence records.
- The VS Code command `MetaFlow: Save Codex Runtime Evidence Template Records` uses the same template scope picker and writes selected runtime evidence scaffold files under `.metaflow/runtime-evidence` only after explicit confirmation, and existing records require overwrite confirmation before replacement.
